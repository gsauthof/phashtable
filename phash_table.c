        ...
        Gms_Phash_Func hfn)
{
    ...
    h->off_table = (Gms_Phash_Elem*) calloc(h->off_table_n, sizeof h->off_table[0]);
    if (!h->off_table)
        return -1;
    h->idx_table = 0;

    uint8_t *ns = (uint8_t*) calloc(h->off_table_n, sizeof ns[0]);
    if (!ns)
        return -1;
    for (uint32_t i = 0; i < n; ++i) {
        uint32_t x = hfn(p, i, 0);
        uint32_t k = (uint64_t)x * h->off_table_n >> 32;
        ++ns[k];
        if (!ns[k]) {
    ...
    uint32_t **vs = (uint32_t**) calloc(h->off_table_n, sizeof vs[0]);
    if (!vs) {
        gms_phash_table_free_helper(ns, 0, 0);
        gms_phash_table_free(h);
        return -1;
    ...
    {
        uint32_t *t = ws;
        for (uint32_t i = 0; i < h->off_table_n; ++i) {
            if (ns[i]) {
                vs[i] = t;
                t += 2 * ns[i];
            }
    ...
    memset(ns, 0, h->off_table_n * sizeof ns[0]);
    ...
    for (uint32_t i = 0; i < h->off_table_n; ++i) {
        ...
        for (uint8_t k = 0; k < ns[i]; ++k) {
            ...
            uint32_t j = (uint64_t)x * h->off_table_n >> 32;
            const Gms_Phash_Elem *o = h->off_table + j;
